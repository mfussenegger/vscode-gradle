...
import { PinnedTasksRootProjectTreeItem, PinnedTaskTreeItem, NoPinnedTasksTreeItem } from ".";
...
const pinnedTasksGradleProjectTreeItemMap: Map<string, PinnedTasksRootProjectTreeItem> = new Map();
...
function buildGradleProjectTreeItem(task: vscode.Task, icons: Icons): void {
    ...
    if (isWorkspaceFolder(task.scope) && isGradleTask(task)) {
        ...
        if (!gradleProjectTreeItem) {
            gradleProjectTreeItem = new PinnedTasksRootProjectTreeItem(path.basename(definition.projectFolder));
...
export class PinnedTasksTreeDataProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
    ...
    private async buildTreeItems(): Promise<vscode.TreeItem[]> {
        ...
        if (!pinnedTasksGradleProjectTreeItemMap.size) {
            ...
            return [...pinnedTasksGradleProjectTreeItemMap.values().next().value.tasks];
        }
    }
}
